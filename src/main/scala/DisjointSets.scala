...
class DisjointSets[T](initialElements : Seq[T] = Nil) {
   ...
   */
  def add(elem : T) = nodes += (elem -> DisjointSets.Node(elem, 0, None))
  ...
  val nodes : Map[T, DisjointSets.Node[T]] = Map.empty
...
object DisjointSets {
  ...
  ////
  // Internal parts
  case class Node[T](elem : T, var rank : Int, var parent : Option[Node[T]]) {
